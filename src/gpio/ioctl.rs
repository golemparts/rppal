...
// Minimum number of interrupt trigger events buffered by the kernel.
// This is only a suggested value. The kernel may alter it.
const EVENT_BUFFER_SIZE: u32 = 1024;
...
impl Default for LineAttribute {
    fn default() -> Self {
        Self {
            ...
            value: LineAttributeValue { flags: 0 },
        }
    }
}
...
impl fmt::Debug for LineAttribute {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LineAttribute")
            ...
            .field("value", unsafe {
                match self.id {
                    LINE_ATTR_ID_FLAGS => &self.value.flags,
                    LINE_ATTR_ID_OUTPUT_VALUES => &self.value.values,
                    LINE_ATTR_ID_DEBOUNCE => &self.value.debounce_period_us,
                    _ => &"",
                }
            })
            .finish()
...
impl LineInfo {
    pub fn new(cdev_fd: c_int, offset: u32) -> Result<LineInfo> {
        let mut line_info = LineInfo {
            ...
            attrs: [LineAttribute::default(); 10],
            padding: [0u32; 4],
        };
...
impl LineEvent {
    pub(crate) fn new(event_fd: c_int) -> Result<LineEvent> {
        let mut line_event = LineEvent::default();

        let bytes_read = parse_retval!(unsafe {
            libc::read(
                event_fd,
                &mut line_event as *mut LineEvent as *mut c_void,
...
impl EventRequest {
    pub fn new(
        ...
        let mut line_request = LineRequest::default();
        line_request.offsets[0] = u32::from(pin);
        line_request.num_lines = 1;

        // Suggest a minimum number of buffered events
        line_request.event_buffer_size = EVENT_BUFFER_SIZE;

        // Set consumer label, so other processes know we're monitoring this event
        line_request.consumer[0..CONSUMER_LABEL.len()].copy_from_slice(CONSUMER_LABEL.as_bytes());

        // Configure triggers
        line_request.config = LineConfig {
            flags: LINE_FLAG_INPUT
                + match trigger {
                    Trigger::Disabled => 0,
    ...
    pub fn close(&mut self) {
        if self.line_request.fd > 0 {
            unsafe {
                libc::close(self.line_request.fd);
            }

            self.line_request.fd = 0;
        }
    }
}
